...
実機を使う場合は、micro:bit V1/V2、USBケーブル、磁石から離れた校正場所を用意します。ネットワークが不安定な授業では依存関係とMakeCode画面を事前に準備してください。2人1台でも進行できます。
...
| 80〜88分 | 実機またはブロック確認 | HEX転送または変換確認 |
...
早く終わった学習者は、センサーが `-1` を返すテストを読み、最後の有効なheadingを維持する理由を考えます。
